...
function nrna_add_executive_committee_meta_boxes()
{
    ...
    add_meta_box(
        'team_type_box',
        __('Team Type', 'nrna'),
        'nrna_render_team_type_meta_box',
        'executive_committee',
        'normal',
        'high'
    );
...
// Render Tenure meta box
function nrna_render_tenure_meta_box($post)
{
    $tenure_from = get_post_meta($post->ID, 'tenure_from', true);
    $tenure_to = get_post_meta($post->ID, 'tenure_to', true);
    echo '<div style="margin-bottom: 15px;">';
    echo '<label for="tenure_from" style="display:block; font-weight:bold; margin-bottom:8px;">From Year:</label>';
    echo '<input type="text" id="tenure_from" name="tenure_from" value="' . esc_attr($tenure_from) . '" style="width:100%;" placeholder="e.g., 2022" />';
    echo '</div>';
    echo '<div>';
    echo '<label for="tenure_to" style="display:block; font-weight:bold; margin-bottom:8px;">To Year:</label>';
    echo '<input type="text" id="tenure_to" name="tenure_to" value="' . esc_attr($tenure_to) . '" style="width:100%;" placeholder="e.g., 2024" />';
    echo '</div>';
}

// Render Team Type meta box
function nrna_render_team_type_meta_box($post)
{
    $team_type = get_post_meta($post->ID, 'team_type', true);
    echo '<label for="team_type" style="display:block; font-weight:bold; margin-bottom:8px;">Team Type:</label>';
    echo '<input type="text" id="team_type" name="team_type" value="' . esc_attr($team_type) . '" style="width:100%;" placeholder="e.g., Executive, Advisory" />';
}
...
function nrna_save_executive_committee_meta_boxes($post_id)
{
    ...
    if (array_key_exists('tenure_from', $_POST)) {
        update_post_meta($post_id, 'tenure_from', sanitize_text_field($_POST['tenure_from']));
    }
    if (array_key_exists('tenure_to', $_POST)) {
        update_post_meta($post_id, 'tenure_to', sanitize_text_field($_POST['tenure_to']));
    }
    if (array_key_exists('team_type', $_POST)) {
        update_post_meta($post_id, 'team_type', sanitize_text_field($_POST['team_type']));
    }
}
...
function nrna_add_executive_committee_columns($columns)
{
    ...
    $columns['tenure'] = __('Tenure', 'nrna');
    $columns['team_type'] = __('Team Type', 'nrna');
    return $columns;
}
...
function nrna_populate_executive_committee_columns($column, $post_id)
{
    ...
    if ($column === 'tenure') {
        $tenure_from = get_post_meta($post_id, 'tenure_from', true);
        $tenure_to = get_post_meta($post_id, 'tenure_to', true);
        if ($tenure_from || $tenure_to) {
            echo esc_html($tenure_from . ' - ' . $tenure_to);
        }
    }
    if ($column === 'team_type') {
        $team_type = get_post_meta($post_id, 'team_type', true);
        echo esc_html($team_type);
    }
}
