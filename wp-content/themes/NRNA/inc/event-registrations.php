...
/**
 * Register REST API endpoint for event registration submissions
 */
function nrna_register_event_registration_endpoint()
{
    register_rest_route('nrna/v1', '/event-registration', [
        'methods' => 'POST',
        'callback' => 'nrna_handle_event_registration_submission',
        'permission_callback' => '__return_true', // Allow public access
    ]);
}
add_action('rest_api_init', 'nrna_register_event_registration_endpoint');

/**
 * Handle event registration submission
 */
function nrna_handle_event_registration_submission($request)
{
    ...
    $subject = 'New Event Registration: ' . $event_name . ' - ' . $first_name . ' ' . $last_name;

    $email_message = "You have received a new event registration:\n\n";
    $email_message .= "Event: " . ($event_name ? $event_name : 'N/A') . "\n";
    $email_message .= "Name: " . $first_name . " " . $last_name . "\n";
    $email_message .= "Email: " . $email . "\n";
    $email_message .= "Phone: " . $phone . "\n";
    $email_message .= "Address: " . ($address ? $address : 'Not provided') . "\n";
...
/**
 * Add Event Registrations admin menu page
 */
function nrna_add_event_registrations_menu()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'event_registrations';
    ...
    $menu_title = 'Event Registrations';
    if ($count > 0) {
    ...
    add_menu_page(
        ...
        'nrna_event_registrations_page', // Callback function
        'dashicons-tickets-alt',         // Icon
        27                               // Position
    );
}
add_action('admin_menu', 'nrna_add_event_registrations_menu');

/**
 * Display event registrations admin page
 */
function nrna_event_registrations_page()
{
    global $wpdb;
    $table_name = $wpdb->prefix . 'event_registrations';
    ...
    if ($action === 'delete' && $id > 0) {
        check_admin_referer('delete_registration_' . $id);
        $wpdb->delete($table_name, ['id' => $id]);
        echo '<script>window.location.href="admin.php?page=event-registrations&message=deleted";</script>';
        exit;
    }

    // Handle Mark as Read Action
    if ($action === 'mark_read' && $id > 0) {
        check_admin_referer('mark_read_registration_' . $id);
        $wpdb->update($table_name, ['status' => 'read'], ['id' => $id]);
        echo '<script>window.location.href="admin.php?page=event-registrations&message=marked_read";</script>';
        exit;
    }

    // Handle Mark ALL as Read Action
    if ($action === 'mark_all_read') {
        check_admin_referer('mark_all_read_registrations');
        $wpdb->update($table_name, ['status' => 'read'], ['status' => 'new']);
        echo '<script>window.location.href="admin.php?page=event-registrations&message=all_marked_read";</script>';
        exit;
    }
...
?>
        <div class="wrap">
            ...
            <a href="admin.php?page=event-registrations" class="page-title-action">Back to List</a>
            <hr class="wp-header-end">

            <div class="card" style="max-width: 800px; margin-top: 20px; padding: 20px;">
                <h2 style="margin-top: 0;"><?php echo esc_html($submission->first_name . ' ' . $submission->last_name); ?></h2>
                <p>
                    <strong>Event:</strong> <?php echo esc_html($submission->event_name ?: 'N/A'); ?><br>
                    ...
                    <strong>Address:</strong> <?php echo esc_html($submission->address ?: 'N/A'); ?><br>
                    <strong>Date:</strong> <?php echo esc_html(date('F j, Y g:i A', strtotime($submission->submitted_at))); ?>
                </p>
                <hr>
                <div style="margin-top: 20px;">
                    <a href="<?php echo wp_nonce_url('admin.php?page=event-registrations&action=delete&id=' . $submission->id, 'delete_registration_' . $submission->id); ?>"
    ...
    <div class="wrap">
        ...
        <a href="<?php echo wp_nonce_url('admin.php?page=event-registrations&action=mark_all_read', 'mark_all_read_registrations'); ?>" class="page-title-action">Mark All as Read</a>
        <hr class="wp-header-end">
            ...
            <table class="wp-list-table widefat fixed striped">
                ...
                <tbody>
                    ...
                    foreach ($submissions as $sub) :
                        ...
                        $view_url = 'admin.php?page=event-registrations&action=view&id=' . $sub->id;
                    ?>
                        <tr class="<?php echo $is_new ? 'updated' : ''; ?>">
                            <td><?php echo $sn++; ?></td>
                            <td><strong><?php echo esc_html($sub->event_name ?: 'N/A'); ?></strong></td>
                            <td>
                                ...
                                <div class="row-actions">
                                    ...
                                    <span class="delete"><a href="<?php echo wp_nonce_url('admin.php?page=event-registrations&action=delete&id=' . $sub->id, 'delete_registration_' . $sub->id); ?>" class="submitdelete" onclick="return confirm('Are you sure?')">Delete</a></span>
                                </div>
                            </td>
